...
Our focus is on the population of dogs in shelters. Focusing on the statistics of certain breed of dogs in the shelter, the gender, age, coat and whether or not they are fixed, to see the duration of how long they tend to stay in the shelter. To address this subject we will compare the statistics provided from the alldogdescription data provided from the pudding dataset with each other.
...
How many features (columns) are in the data? 36  
What, if any, ethical questions or questions of power do you need to consider when working with this data?  
  Some ethical questions to consider when working with this data are how is the adoptability determined, who determines the adoptability, and are the criteria for adoptability the same across the dataset. Additionally, there may be other factors that can help to determine.
...
Technologists, designers, and policymakers can observe which dog demographic (e.g., dog age and coat) are most common in dog shelters. An implication they can draw from the data is which qualities of the dogs are the most and least desirable, as a high population of a type of dog in shelters imply low desirability/demand among potential dog owners. Another implication from the data for dog shelters is to adjust the price of dogs according to the demand for the qualities of the dogs; a dog with a certain age and type of coat can be priced higher or lower considering its degree of demand by looking at our data. Policymakers can take the data and consider enacting equity-focused legislative reforms that advocate for the types of dogs that are lower in demand and more likely to end up in shelters to enable those dogs are more likely to find a forever home. 
...
In the table, we narrowed the variables down to the 5 different factors that we are evaluating to see if there is a correlation between certain characteristics and how it affects their adoptability. These variables include age, gender, breed, coat (fur/hair length), and date they were posted. We kept only 50 observations out of the 58180, arranged starting with the ones posted the earliest, so there were enough dogs and traits to compare to each other but reduced the number significantly to keep it simple and organized. 
